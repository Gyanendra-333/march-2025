...
const Cart = () => {
    const { navigate, products, currency, addToCart, updateCartQuantity, removeFromCart, cartItems, searchQuery, setSearchQuery, getCartCount, getCartAmount } = useAppContext();
    ...
    return products.length > 0 && cartItems ? (
        <div className="flex flex-col md:flex-row py-16 mt-16 max-w-6xl w-full px-6 mx-auto">
                            ...
                            <div>
                                ...
                                <div className="font-normal text-gray-500/70">
                                    ...
                                    <div className='flex items-center'>
                                        <p>Qty:</p>
                                        <select onChange={(e) => updateCartQuantity(product._id, Number(e.target.value))} value={cartItems[product._id]} className='outline-none'>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {addresses?.map((address, index) => (
                                    <p key={index} onClick={() => { selectAddress(address); setShowAddress(false) }} className="text-gray-500 p-2 hover:bg-gray-100">
                ...
                <button onClick={placeOrder()} className="w-full py-3 mt-6 cursor-pointer bg-primary text-white font-medium hover:bg-primary-dull transition">
                    {paymentOption === "COD" ? "Place Order" : "Preceed to checkout"}
                </button>
            </div>
        </div>
    ) : (null)
